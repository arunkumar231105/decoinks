  ...
  return (
    <div className="nq-sidebar-card">
      ...
      <div className="nq-terms-grid">
        ...
        <div className="nq-terms-field"><label>Payment Method</label><select className="nq-select" value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)}><option>Bank Transfer</option><option>Zelle</option><option>PayPal</option><option>Stripe</option><option>Shopify</option><option>Cash App</option><option>Credit Card</option><option>Cash</option><option>Check</option></select></div>
      </div>
    </div>
  )
}
...
export function NewQuotationPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (res: any) => {
      ...
      if (quoteId) {
        ...
        // Line artwork is listed in the Artworks card too
        queryClient.invalidateQueries({ queryKey: ['quote-artworks', quoteId] })
      }
  ...
  // A line that came in without a style — an import, or a quote written before
  ...
  const linkStyleToItem = (itemId: string, style: CatalogStyle) => {
    setApparelItems(prev => prev.map(item => {
      ...
      const colour = style.colors?.find(c =>
        c.display_name.toLowerCase() === (item.variant || '').trim().toLowerCase())
      const size = style.sizes?.find(z =>
        z.size_name.toLowerCase() === (item.sizes || '').trim().toLowerCase())
      ...
      return {
        ...
        styleCode:        style.sku,
        brand:            style.brand,
        productImage:     style.images?.[0]?.image_url ?? style.image_url ?? item.productImage,
        styleDescription: style.description ?? item.styleDescription,
        ...
        variant:          colour?.display_name ?? item.variant,
        sizes:            size?.size_name ?? item.sizes,
