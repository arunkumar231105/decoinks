...
export default function ArtworkUploader({ orderId, quotationId }: Props) {
  const queryClient  = useQueryClient()
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [dragging, setDragging]   = useState(false)
  const [uploading, setUploading] = useState(false)

  // Determine endpoint prefix based on what ID is provided
  const base     = orderId ? `/orders/${orderId}` : `/quotations/${quotationId}`
  const queryKey = orderId ? ['order-artworks', orderId] : ['quote-artworks', quotationId]
  const enabled  = !!(orderId || quotationId)

  // ── Artworks list ──────────────────────────────────────────────────────────
  const { data: artworkData } = useQuery<{ artworks: Artwork[] }>({
    queryKey,
    queryFn: () => api.get(`${base}/artworks`).then(r => r.data),
    enabled,
  })
  const artworks = artworkData?.artworks ?? []

  // ── Gangsheet (only for orders) ────────────────────────────────────────────
  ...
  const deleteMutation = useMutation({
    mutationFn: (artworkId: string) => api.delete(`${base}/artworks/${artworkId}`),
    onSuccess: () => {
  ...
  const uploadFile = useCallback(async (file: File) => {
    if (!enabled) {
      toast.error('Save the record first before uploading artworks')
      return
    }
  ...
  }, [base, enabled, queryClient, queryKey])
  ...
  const thumbUrl = (aw: Artwork) => aw.file_url || null
    ...
    <div className="od-card">
      ...
      {!enabled && (
        <p style={{ fontSize: 12, color: '#9ca3af', textAlign: 'center', padding: '16px 0' }}>
          Save the record first to enable artwork uploads
        </p>
      )}
          ...
          {artworks.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 10 }}>
              {artworks.map((aw) => {
                ...
                return (
                  <div key={aw.id} style={{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden', position: 'relative', background: '#f9fafb' }}>
