...
export function LeadDetailsDrawer({ leadId, onClose }: { leadId: string | null; onClose: () => void }) {
  ...
  const qualificationLabels: Array<[string, string]> = [
    ['customer_responded', 'Customer responded'], ['human_engaged', 'Human engaged'], ['product_identified', 'Product identified'],
    ['artwork_received', 'Artwork received'], ['quantity_discussed', 'Quantity discussed'], ['sizes_received', 'Sizes received'],
    ['shipping_address_confirmed', 'Shipping address received'], ['delivery_date_confirmed', 'Timeline discussed'],
    ['quote_requested', 'Quote requested'], ['mockup_requested', 'Mockup requested'], ['budget_confirmed', 'Budget discussed'],
  ]
  return <>
